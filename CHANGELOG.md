...
## 2026-08-21 — UI 文案去 AI 痕迹 + README 全页面截图补全

### 用户可见文案清理（ux）

- 清理全部用户可见的 "AI" 字样：meta 描述、浏览器标签、加载屏（"AI 专家研判"/"AI 深度分析"）、导出报告脚注、SSE 进度（"8 位 AI 专家"→"8 位专家"）、OpenAPI 描述（"多专家 AI 仲裁/研判"→"多专家仲裁/研判"）——统一为"多专家"中性表述。
- 工程文档同步去案底：CHANGELOG/ENGINEERING-NOTES 的"AI 味诊断"章节 →"模板感诊断"；CODE_REVIEW_REPORT "llm/ AI 层"→"模型层"。
- 修复模拟盘空账户显示瑕疵（"当前交易日：，可用现金" → currentDate 为空时显示"未设置"）。

### README 界面展示补全（docs）

- 界面展示由 2 页 4 图扩为 **5 页 10 图**：新增对比分析、自选股、模拟盘、研究助手、研究历史 5 页真实浏览器截图（数据态：自选股 2 只、模拟盘已下单并结算、助手含降级应答、历史含记录），截图经视觉审查确认**无任何 "AI" 字样**。
