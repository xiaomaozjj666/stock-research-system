...
## 附：数据面扩容批次（2026-09-12）

两轮递进批次（数据途径实测研究 → 全链路落地），全部「先 curl/直调拿 ground truth，再编码」：

### 批次 A：组合回测撮合升级 + 两融因子 + 一致预期（bd00780）

- **组合回测 T+1 撮合**：从「t 收盘决策、同一收盘价成交」改为「t 收盘决策 → t+1
  开盘建仓 → t+1+holdDays 开盘平仓」，同时消除同 bar 决策-成交前视与 A 股 T+1
  卖出约束两处乐观偏差；基准同口径；成交日缺开盘价（停牌）剔除出分母；最少数据
  要求由 2×holdDays 放宽为 holdDays+2。涨停仍不建模——按板块写死阈值会误判
  创业板 20% 涨跌幅（过度剔除的偏差可能比不建模更大），如实告知而非错误建模。
- **两融 PIT 因子族**（marginProvider）：RPTA_WEB_RZRQ_GGMX 日度序列；
  mg_balance_chg20（融资余额 20 日变化率）+ mg_balance_pct（拥挤度）。
  **T+1 披露延迟**：交易所两融数据 T 日交易 T+1 盘前披露，t 日因子值强制只用
  严格早于 t 的行（marginFactorValues 内执行）。
- **机构一致预期快照**（consensusProvider）：RPT_WEB_RESPREDICT（覆盖机构/评级
  分布/EPS A-E/目标价）+ 北向季度持股；接入 8 位专家 LLM 语境与结果页卡片。
  方法论纪律：快照无历史序列，严禁当回测因子——把今天的预期投影回历史即前视。
- 客户端：截面页两融开关/类型标签（并补齐此前缺失的「形态」标签）、撮合文案更新。

### 批次 B：Tushare token 配置 + 频控纪律（340dce5）

- 免费积分实测：stock_basic 可用（L=5562）但频控随用量升级（1次/分钟 → 1次/小时
  → 5次/天）；index_weight 无权限（历史成分缺口当时无解）。
- 适配器加 `*Cached` 包装：24h 缓存（env 可覆盖）+ 上游失败回落陈旧缓存 + 并发
  去重；请求路径禁止直连裸函数。接线：health tushare 块（配置态/计数/降级披露）、
  幸存者声明附退市股名单规模。token 经 server/.env 注入（gitignored，提交前
  全仓库泄漏扫描）。

### 批次 C：Baostock Python sidecar——幸存者偏差正面修复（f4a784d）

- 指数历史成分快照（hs300/zz500/sz50 任意日期，**含其后退市证券**；实测 2015
  沪深300 成分含 2016 年退市的武钢股份）——东财免费通道与 Tushare 免费积分都
  给不了的数据。
- sidecar（stdin/stdout JSON 协议、噪声重定向 stderr、chdir 临时目录防日志污染）
  - Node 桥接（60s 超时 / ENOENT 友好指引 / 不可变快照 30 天缓存 / 陈旧兜底 /
    并发去重）。
- indexUniverse 源接入截面/表达式/批量三路由 + MCP + 截面页 UI；幸存者声明按
  宇宙来源分口径（index 源「含其后退市证券，缺 K 线者如实跳过」）并首次上屏。

### 收尾时门禁（2026-09-12 更新）

- 1458 单测（130 文件）、lint / prettier / 双端 tsc / build / CI 全绿
- 实机：sz50@2024-06-28 历史成分截面（50 只入组、24 因子）；health 双增强通道
  如实披露（baostock available / tushare 降级原因）
