...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-2890%20cases-brightgreen" alt="2890 测试用例" />
...
**因子研究闭环（截面评估 · 因子实验室 · 组合回测）**
五族真实数据因子（量价 / 基本面 PIT / 事件：分红·回购·解禁·PEAD·龙虎榜 / 技术形态 / 两融）按日截面评估：Spearman IC + Newey-West HAC + Holm 多重检验 + 分层收益 + Walk-Forward 样本外稳定性。受限 DSL 因子实验室：白名单表达式（无 eval 无注入面）、批量假设验证共享面板、实验台账带 FDR 折扣（全历史试错的期望假阳性如实计入）。因子组合回测：top-N 等权周期调仓、A 股真实成本、**T+1 次日开盘撮合**（t 收盘决策、t+1 开盘成交，消除同 bar 决策-成交前视），候选宇宙等权为因子中性基准。

**多数据通道（免费为主，可选增强）**
东方财富免费接口族（行情/财务/事件/两融/盈利预测）为主通道；可选增强：Tushare Pro（退市股名单，**频控纪律缓存**：长缓存 + 失败回落陈旧 + 并发去重）与 Baostock Python sidecar（**指数历史成分**：hs300/zz500/sz50 任意日期快照，含其后退市的证券——幸存者偏差的正面修复，截面评估可直接用历史成分宇宙）。所有增强通道未配置时优雅跳过，绝不反向依赖。

**机构一致预期（快照语境）**
分析师覆盖机构数、评级分布、逐年度 EPS 预测与目标价区间、北向季度持股，注入八位专家研判语境并在报告页展示；明示快照口径、不参与历史回测（把今天的预期投影回过去即前视）。
...
| 时间序列计量        | statsmodels（纯函数重实现）         | `quant/timeseries/`：ADF 单位根、GARCH(1,1)+EGARCH(1,1) 波动率、Engle-Granger 协整（价差半衰期）、ARIMA(p,d,0)、Kalman 时变对冲比率 |
...
- 测试：Vitest（服务 / 量化 / 研究 Agent / 前端组件，2890 用例 / 212 个测试文件，行覆盖 94.4%）+ Playwright（E2E 9 用例）+ GitHub Actions CI（质量门禁 + 覆盖率阈值 + E2E）
...
```bash
npm test              # Vitest 全量单测（2890 用例 / 212 个测试文件：服务 / 量化 / 研究 Agent / 前端组件）
