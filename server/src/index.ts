...
import { httpMetricsMiddleware, renderPrometheus } from './services/metrics.js';
...
// 生产分析管线会产生 trade_signal 审计条目，连续强信号将触发熔断以阻止风险扩散。
function circuitBreakerGuard(req: Request, res: Response, next: NextFunction) {
...
// 核心分析接口
app.post('/api/analyze', analyzeLimiter, circuitBreakerGuard, async (req, res) => {
  try {
...
// 股票对比接口
app.post('/api/compare', compareLimiter, circuitBreakerGuard, async (req, res) => {
  try {
...
app.post('/api/quant/analyze', quantLimiter, circuitBreakerGuard, async (req, res) => {
  try {
...
app.post('/api/backtest/evaluate', watchlistLimiter, circuitBreakerGuard, async (req, res) => {
  try {
    const body = req.body ?? {};
...
// === 对话式智能体接口（自然语言入口） ===
app.post('/api/chat', chatLimiter, circuitBreakerGuard, async (req, res) => {
  try {
    const body = req.body ?? {};
...
app.post('/api/watchlist/monitor', watchlistLimiter, circuitBreakerGuard, async (req, res) => {
  try {
