...
function makeEchoServerScript(toolName: string): string {
  return [
    ...
    "    send(msg.id,{tools:[{name:'" + toolName + "',description:'echo tool: " + toolName + "',inputSchema:{type:'object',properties:{text:{type:'string',description:'input text'}},required:['text']}}]});",
