...
describe('calculateSectorRotation - 综合评分加权', () => {
  ...
  it('综合评分 ∈ [0, 100]', () => {
    const sectors = [
      makeSector({ sector: 'A', revenueGrowth: 100, profitGrowth: 200, roeChange: 10,
                   momentum20d: 50, momentum60d: 100, benchmarkMomentum20d: -20,
                   turnoverRate: 0.1, volumeRatio: 0.01, northboundConcentration: 0 }),
      makeSector({ sector: 'B', revenueGrowth: -50, profitGrowth: -80, roeChange: -10,
                   momentum20d: -50, momentum60d: -100, benchmarkMomentum20d: 20,
                   turnoverRate: 50, volumeRatio: 30, northboundConcentration: 10 }),
    ];
    const result = calculateSectorRotation(sectors);
    for (const sig of result.signals) {
...
describe('calculateSectorRotation - 排名与推荐分类', () => {
  ...
  const sectors: SectorData[] = [
    makeSector({
      sector: 'Top',
      revenueGrowth: 30, profitGrowth: 40, roeChange: 3,
      momentum20d: 10, momentum60d: 20, benchmarkMomentum20d: 0,
      turnoverRate: 1, volumeRatio: 0.5, northboundConcentration: 0.1,
    }),
    makeSector({
      sector: 'Good',
      revenueGrowth: 15, profitGrowth: 18, roeChange: 1,
      momentum20d: 5, momentum60d: 8, benchmarkMomentum20d: 0,
      turnoverRate: 2, volumeRatio: 1, northboundConcentration: 0.3,
    }),
    makeSector({
      sector: 'Mid',
      revenueGrowth: 5, profitGrowth: 5, roeChange: 0,
      momentum20d: 0, momentum60d: 0, benchmarkMomentum20d: 0,
      turnoverRate: 3, volumeRatio: 1.5, northboundConcentration: 0.5,
    }),
    makeSector({
      sector: 'Bad',
      revenueGrowth: -5, profitGrowth: -10, roeChange: -1,
      momentum20d: -5, momentum60d: -8, benchmarkMomentum20d: 0,
      turnoverRate: 4, volumeRatio: 2, northboundConcentration: 1,
    }),
    makeSector({
      sector: 'Bottom',
      revenueGrowth: -20, profitGrowth: -30, roeChange: -3,
      momentum20d: -10, momentum60d: -20, benchmarkMomentum20d: 0,
      turnoverRate: 8, volumeRatio: 4, northboundConcentration: 2,
    }),
  ];
  const result = calculateSectorRotation(sectors);
...
describe('calculateSectorRotation - 边界场景', () => {
  it('仅 1 个行业：rank=1，recommendation=overweight', () => {
    const result = calculateSectorRotation([
      makeSector({
        sector: 'Only',
        revenueGrowth: 10, profitGrowth: 10, roeChange: 0,
        momentum20d: 5, momentum60d: 10, benchmarkMomentum20d: 0,
        turnoverRate: 2, volumeRatio: 1, northboundConcentration: 0.5,
      }),
    ]);
...
describe('calculateSectorRotation - 输出结构', () => {
  it('返回的 SectorSignal 字段完整', () => {
    const result = calculateSectorRotation([
      makeSector({ sector: 'A', revenueGrowth: 10, profitGrowth: 10, roeChange: 1,
                   momentum20d: 5, momentum60d: 10, benchmarkMomentum20d: 0,
                   turnoverRate: 2, volumeRatio: 1, northboundConcentration: 0.5 }),
      makeSector({ sector: 'B', revenueGrowth: 5, profitGrowth: 5, roeChange: 0,
                   momentum20d: 2, momentum60d: 4, benchmarkMomentum20d: 0,
                   turnoverRate: 3, volumeRatio: 1.5, northboundConcentration: 0.8 }),
    ]);
