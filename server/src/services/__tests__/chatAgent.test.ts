...
describe('chatAgent — 路由规划（planner/router）', () => {
  ...
  it('规划器返回 tools 时走 chatWithTools', async () => {
    const agent = createChatAgent(baseDeps({
      ...
      chatWithTools: async () => ({ content: '分析结果', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
  ...
  it('规划器异常时降级为 tools', async () => {
    const agent = createChatAgent(baseDeps({
      isLLMAvailable: () => true,
      chatJSON: async () => { throw new Error('LLM 挂了'); },
      chatWithTools: async () => ({ content: 'ok', toolCalls: [] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
    expect(r.plan?.action).toBe('tools');
...
describe('chatAgent — 幻觉防护（事实校验）', () => {
  it('有工具结果时触发校验，verified=true 时不警示', async () => {
    ...
    const agent = createChatAgent(baseDeps({
      ...
      chatWithTools: async () => ({ content: '茅台评分80', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
  ...
  it('校验发现未验证断言时标记 verified=false 并给警示', async () => {
    ...
    const agent = createChatAgent(baseDeps({
      ...
      chatWithTools: async () => ({ content: '茅台明年涨到2000元', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
  ...
  it('计算型声明算术重构：发现计算错误时填入 calculationErrors', async () => {
    ...
    const agent = createChatAgent(baseDeps({
      ...
      chatWithTools: async () => ({ content: '营收增长30%', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
    expect(r.verification?.verified).toBe(false);
  ...
  it('calculationErrors 为空且 unverified 为空时 verified=true', async () => {
    ...
    const agent = createChatAgent(baseDeps({
      ...
      chatWithTools: async () => ({ content: '分析正确', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
    expect(r.verification?.verified).toBe(true);
  ...
  it('chatJSON 缺失时跳过校验', async () => {
    const agent = createChatAgent(baseDeps({
      isLLMAvailable: () => true,
      chatWithTools: async () => ({ content: '分析', toolCalls: [{ name: 'run_analysis', args: {} }] }),
    }));
    const r = await agent.run({ message: '分析 600519' });
    expect(r.verification).toBeUndefined();
  });
...
describe('chatAgent — 风控三分视角辩论', () => {
  ...
  it('风控辩论失败时不影响主回答', async () => {
    const runRiskDebate = vi.fn(async () => { throw new Error('辩论服务不可用'); });
    const agent = createChatAgent(baseDeps({
      isLLMAvailable: () => true,
      chatJSON: async () => ({ action: 'tools', reason: '分析' }),
      chatWithTools: async () => ({ content: '主回答', toolCalls: [{ name: 'run_analysis', args: {} }] }),
      runRiskDebate,
    }));
  ...
  it('risk_debating 阶段事件在风控辩论时推送', async () => {
    const events: string[] = [];
    const runRiskDebate = vi.fn(async () => ({
      aggressive: '', neutral: '', conservative: '', synthesis: '',
    }));
    const agent = createChatAgent(baseDeps({
      isLLMAvailable: () => true,
      chatJSON: async () => ({ action: 'tools', reason: '分析' }),
...
describe('chatAgent — 流式阶段事件（runStream）', () => {
  ...
  it('异常时推送 error 事件并 rethrow', async () => {
    ...
    const agent = createChatAgent(baseDeps({
      isLLMAvailable: () => true,
      retrieveEvidence: async () => { throw new Error('检索崩溃'); },
    }));
    await expect(
      agent.runStream({ message: '分析' }, (e) => events.push(e.phase)),
    ).rejects.toThrow('检索崩溃');
