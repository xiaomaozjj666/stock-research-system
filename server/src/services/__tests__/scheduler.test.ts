...
describe('autonomous 循环', () => {
        ...
        results: [
          {
            code: '600519',
            name: '茅台',
            newsSentiment: { polarity: 0.8, weightedImpact: 0.3, hasNews: true },
          },
        ],
        count: 1,
        ...
        results: [
          {
            code: 'x',
            name: null,
            newsSentiment: { polarity: 0.1, weightedImpact: 0.1, hasNews: true },
          },
        ],
        count: 1,
        generatedAt: new Date().toISOString(),
  ...
  it('单次监控失败不终止循环', async () => {
    let n = 0;
    const monitor = vi.fn(
      async (): Promise<{ results: WatchlistAlertInput[]; count: number; generatedAt: string }> => {
        n++;
