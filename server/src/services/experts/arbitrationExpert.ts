...
function arbitrationExpertRule(input: ArbitrationInput): {
  ...
  for (const theme of divergenceThemes) {
    const supporters = opinions.filter(o => o.arguments.some(a => a.text.includes(theme) && a.type === 'support'));
    const opposers = opinions.filter(o => o.arguments.some(a => a.text.includes(theme) && a.type === 'oppose'));
  ...
  const latestGrowth = financial.netProfit[n - 2] !== 0
    ? (financial.netProfit[n - 1] - financial.netProfit[n - 2]) / Math.abs(financial.netProfit[n - 2]) * 100
    : 0;
  const avgGrowth = (n >= 2 && financial.netProfit[0] > 0 && financial.netProfit[n - 1] > 0)
  ...
  const revenueGrowthLatest = financial.revenue[n - 2] !== 0
    ? (financial.revenue[n - 1] - financial.revenue[n - 2]) / Math.abs(financial.revenue[n - 2]) * 100
    : 0;
  const revenueGrowthPrev = n >= 3 && financial.revenue[n - 3] !== 0
    ? (financial.revenue[n - 2] - financial.revenue[n - 3]) / Math.abs(financial.revenue[n - 3]) * 100
    : 0;

  const hasPeerData = valuation.peerComparison.length > 0;
  const peerAvgPE = hasPeerData
    ? safeDiv(valuation.peerComparison.reduce((s, p) => s + p.pe, 0), valuation.peerComparison.length)
    : 0;
  ...
  if (hasPeerData) {
    arbValuation += valuation.pe > peerAvgPE
  ...
  const growthForecast = latestGrowth > 15 ? '中高速'
    : latestGrowth > 10 ? '中速' : latestGrowth > 5 ? '中低速' : '低速';
  const growthSupport = grossMarginLatest > 30 ? '高毛利率提供利润缓冲' : '毛利率偏低需关注成本控制';
  ...
  controversies.push({
    ...
    confidence: Math.round(60 + (cashFlowGood ? 10 : cashFlowBad ? -5 : 5) + (marginStable ? 5 : -5) + (debtHigh ? -5 : 5))
  });
  ...
  const competitionTrend = marginTrendUp ? '竞争格局改善' : marginTrendDown ? '竞争加剧' : '竞争格局稳定';
  ...
  // === 最终综合判断 ===
  const avgConfidence = safeDiv(opinions.reduce((s, o) => s + o.confidence, 0), opinions.length);
  ...
  const fundamentalLevel = avgROE > 20 && grossMarginLatest > 30 ? '优秀'
    : avgROE > 10 && grossMarginLatest > 20 ? '良好'
    : avgROE > 5 && grossMarginLatest > 10 ? '中等'
    : '偏弱';

  // 动态定位建议
  const positioningAdvice = bullishCount >= 3 && bearishCount <= 1 ? '优先跟踪'
    : bullishCount >= 2 ? '持续观察'
    : bearishCount >= 3 ? '建议规避'
    : '谨慎观望';
...
export async function arbitrationExpert(input: ArbitrationInput): Promise<{
  ...
  // 构建专家意见摘要供 LLM 仲裁
  const opinionsBrief = input.opinions.map(o => {
  ...
  const messages: ChatMessage[] = [
    ...
    { role: 'user', content: `${formatContext(input.financial, input.valuation, input.info)}\n\n=== 各专家研判意见 ===\n${opinionsBrief}\n\n${ARBITRATION_SCHEMA}` },
  ];
  ...
  const arguments_ = Array.isArray(fo.arguments)
    ? fo.arguments.map((a) => {
        ...
        return {
          ...
          type: validTypes.includes(arg.type as typeof validTypes[number]) ? (arg.type as 'support' | 'oppose') : 'support',
          evidenceType: validEvidence.includes(arg.evidenceType as typeof validEvidence[number]) ? (arg.evidenceType as 'fact' | 'inference' | 'hypothesis') : 'inference',
        };
      }).filter(a => a.text)
    : [];
  const sentiment = validSentiments.includes(fo.overallSentiment as typeof validSentiments[number])
    ? (fo.overallSentiment as 'bullish' | 'neutral' | 'bearish') : 'neutral';
  const keyPoints = Array.isArray(fo.keyPoints) ? fo.keyPoints.map(p => String(p)).filter(p => p) : [];
