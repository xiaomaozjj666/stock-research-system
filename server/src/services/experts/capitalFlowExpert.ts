 ...
 * 规则引擎研判（LLM 不可用时降级使用）
 */
function capitalFlowExpertRule(financial: FinancialData, valuation: ValuationData, _info: StockInfo): ExpertOpinion {
  ...
  if (valuation.peerComparison.length > 0) {
    ...
    const peerAvgPE = safeDiv(valuation.peerComparison.reduce((s, p) => s + p.pe, 0), valuation.peerComparison.length);
  ...
  const capDisplay = capYi >= 10000
    ? `${(capYi / 10000).toFixed(2)}万亿元`
    : `${capYi.toFixed(0)}亿元`;
  const capDisplayShort = capYi >= 10000
    ? `${(capYi / 10000).toFixed(2)}万亿`
    : `${capYi.toFixed(0)}亿`;
 ...
 */
export async function capitalFlowExpert(financial: FinancialData, valuation: ValuationData, info: StockInfo): Promise<ExpertOpinion> {
