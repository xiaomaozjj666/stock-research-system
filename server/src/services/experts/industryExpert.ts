...
function calcIndustryScore(financial: FinancialData): number {
  ...
  for (let i = 1; i < n; i++) {
    revenueGrowthRates.push(safeDiv(financial.revenue[i] - financial.revenue[i - 1], financial.revenue[i - 1]) * 100);
  }
  const avgRevenueGrowth = revenueGrowthRates.length > 0
    ? revenueGrowthRates.reduce((a, b) => a + b, 0) / revenueGrowthRates.length : 0;
 ...
 * 规则引擎研判（LLM 不可用时降级使用）
 */
function industryExpertRule(financial: FinancialData, valuation: ValuationData, _info: StockInfo): IndustryExpertResult {
  const n = financial.years.length;
  ...
  if (avgRevenueGrowth > 15) {
    arguments_.push({ text: `行业平均营收增速${avgRevenueGrowth.toFixed(1)}%，处于高速成长期`, confidence: 78, type: 'support', evidenceType: 'inference' });
    keyPoints.push(`行业处于高速成长期，平均增速${avgRevenueGrowth.toFixed(0)}%`);
  } else if (avgRevenueGrowth > 5) {
    arguments_.push({ text: `行业平均营收增速${avgRevenueGrowth.toFixed(1)}%，处于稳健增长阶段`, confidence: 70, type: 'support', evidenceType: 'inference' });
  } else if (avgRevenueGrowth > 0) {
    arguments_.push({ text: `行业平均营收增速仅${avgRevenueGrowth.toFixed(1)}%，行业进入低增长或成熟期`, confidence: 68, type: 'oppose', evidenceType: 'inference' });
    keyPoints.push(`行业增速放缓至${avgRevenueGrowth.toFixed(1)}%，进入成熟期`);
  } else {
    arguments_.push({ text: `行业平均营收增速${avgRevenueGrowth.toFixed(1)}%，行业整体收缩`, confidence: 75, type: 'oppose', evidenceType: 'inference' });
    keyPoints.push(`行业整体收缩，平均增速${avgRevenueGrowth.toFixed(1)}%`);
  }

  if (growthTrend > 5) {
    arguments_.push({ text: `最新营收增速${latestRevenueGrowth.toFixed(1)}%高于均值${avgRevenueGrowth.toFixed(1)}%，行业景气度回升`, confidence: 72, type: 'support', evidenceType: 'inference' });
  } else if (growthTrend < -5) {
    arguments_.push({ text: `最新营收增速${latestRevenueGrowth.toFixed(1)}%低于均值${avgRevenueGrowth.toFixed(1)}%，行业景气度下行`, confidence: 75, type: 'oppose', evidenceType: 'inference' });
  ...
  if (grossMarginTrend > 3) {
    arguments_.push({ text: `毛利率${financial.grossMargin[0].toFixed(1)}%→${financial.grossMargin[n - 1].toFixed(1)}%，呈上升趋势，行业竞争格局改善或定价权增强`, confidence: 75, type: 'support', evidenceType: 'inference' });
  } else if (grossMarginTrend < -3) {
    arguments_.push({ text: `毛利率${financial.grossMargin[0].toFixed(1)}%→${financial.grossMargin[n - 1].toFixed(1)}%，呈下降趋势，行业竞争加剧或成本压力上升`, confidence: 75, type: 'oppose', evidenceType: 'inference' });
    keyPoints.push(`毛利率趋势下行${grossMarginTrend.toFixed(1)}个百分点，竞争加剧`);
  } else {
    arguments_.push({ text: `毛利率波动仅${grossMarginTrend.toFixed(1)}个百分点，行业竞争格局稳定`, confidence: 68, type: 'support', evidenceType: 'inference' });
  }
  ...
  if (valuation.peerComparison.length > 0) {
    ...
    const topPeerShare = safeDiv(Math.max(...valuation.peerComparison.map(p => p.marketCap)), valuation.marketCap + totalPeerCap) * 100;
    const concentrationRatio = companyShare + topPeerShare;

    if (concentrationRatio > 60) {
      arguments_.push({ text: `行业集中度高（前两大市值占比${concentrationRatio.toFixed(0)}%），龙头格局清晰，马太效应显著`, confidence: 78, type: 'support', evidenceType: 'inference' });
      keyPoints.push(`行业集中度高，龙头优势显著`);
    } else if (concentrationRatio > 40) {
      arguments_.push({ text: `行业集中度中等（前两大市值占比${concentrationRatio.toFixed(0)}%），竞争格局尚可`, confidence: 65, type: 'support', evidenceType: 'inference' });
    } else {
      arguments_.push({ text: `行业集中度较低（前两大市值占比${concentrationRatio.toFixed(0)}%），竞争分散`, confidence: 68, type: 'oppose', evidenceType: 'inference' });
    }

    // ROE 同业对比需同业 ROE 数据可用（当前行情接口未提供同业 ROE，全为0时跳过，避免误判"领先同业"）
    const hasPeerRoe = valuation.peerComparison.some(p => p.roe > 0);
    if (hasPeerRoe) {
      const peerAvgROE = safeDiv(valuation.peerComparison.reduce((s, p) => s + p.roe, 0), valuation.peerComparison.length);
      const avgROE = financial.roe.reduce((a, b) => a + b, 0) / n;
      if (avgROE > peerAvgROE * 1.3) {
        arguments_.push({ text: `ROE ${avgROE.toFixed(1)}%显著领先同业均值${peerAvgROE.toFixed(1)}%，竞争优势突出`, confidence: 80, type: 'support', evidenceType: 'fact' });
      } else if (avgROE < peerAvgROE * 0.7) {
        arguments_.push({ text: `ROE ${avgROE.toFixed(1)}%低于同业均值${peerAvgROE.toFixed(1)}%，竞争力不足`, confidence: 72, type: 'oppose', evidenceType: 'fact' });
      }
    }
  }
 ...
 */
export async function industryExpert(financial: FinancialData, valuation: ValuationData, info: StockInfo): Promise<IndustryExpertResult> {
