 ...
 * 规则引擎研判（LLM 不可用时降级使用）
 */
function riskExpertRule(financial: FinancialData, _valuation: ValuationData, _info: StockInfo): ExpertOpinion {
  ...
  const arGrowth = n >= 2 ? safeDiv(financial.accountsReceivable[n - 1] - financial.accountsReceivable[n - 2], Math.abs(financial.accountsReceivable[n - 2])) * 100 : 0;
  const revenueGrowth = n >= 2 ? safeDiv(financial.revenue[n - 1] - financial.revenue[n - 2], financial.revenue[n - 2]) * 100 : 0;
  const arRatio = safeDiv(financial.accountsReceivable[n - 1], financial.revenue[n - 1]) * 100;

  if (arGrowth > revenueGrowth + 30 && arGrowth > 20) {
    arguments_.push({ text: `应收账款增速${arGrowth.toFixed(1)}%远超营收增速${revenueGrowth.toFixed(1)}%，存在虚增收入或回款恶化风险`, confidence: 82, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`应收增速${arGrowth.toFixed(0)}%远超营收${revenueGrowth.toFixed(0)}%，回款异常`);
  } else if (arRatio < 3) {
    arguments_.push({ text: `应收账款占营收仅${arRatio.toFixed(1)}%，坏账风险极低`, confidence: 80, type: 'support', evidenceType: 'fact' });
    keyPoints.push(`应收占营收${arRatio.toFixed(1)}%，回款能力极强`);
  } else if (arRatio > 20) {
    arguments_.push({ text: `应收账款占营收${arRatio.toFixed(1)}%，信用风险敞口较大`, confidence: 75, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`应收占营收${arRatio.toFixed(1)}%，回款风险偏高`);
  }

  // === 存货异常检查（存货增速 vs 营收增速） ===
  const inventoryGrowth = n >= 2 ? safeDiv(financial.inventory[n - 1] - financial.inventory[n - 2], Math.abs(financial.inventory[n - 2])) * 100 : 0;

  if (inventoryGrowth > revenueGrowth + 30 && inventoryGrowth > 20) {
    arguments_.push({ text: `存货增速${inventoryGrowth.toFixed(1)}%远超营收增速${revenueGrowth.toFixed(1)}%，存在积压或减值风险`, confidence: 78, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`存货增速${inventoryGrowth.toFixed(0)}%远超营收，积压风险`);
  } else if (inventoryGrowth < 0 && revenueGrowth > 0) {
    arguments_.push({ text: `存货同比下降${Math.abs(inventoryGrowth).toFixed(1)}%而营收增长，库存去化健康`, confidence: 72, type: 'support', evidenceType: 'fact' });
  }
  ...
  if (avgCashFlowRatio < 0.3) {
    arguments_.push({ text: `经营现金流/净利润均值仅${avgCashFlowRatio.toFixed(2)}，利润含金量严重不足`, confidence: 85, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`现金流/净利润${avgCashFlowRatio.toFixed(2)}，利润含金量差`);
    hasCriticalRisk = true;
  } else if (latestCashFlowRatio < 0 && avgCashFlowRatio > 0.5) {
    arguments_.push({ text: `最新年度现金流/净利润转负（${latestCashFlowRatio.toFixed(2)}），盈利质量出现恶化信号`, confidence: 78, type: 'oppose', evidenceType: 'inference' });
  } else if (avgCashFlowRatio > 0.9) {
    arguments_.push({ text: `经营现金流/净利润均值${avgCashFlowRatio.toFixed(2)}，利润含金量高`, confidence: 78, type: 'support', evidenceType: 'fact' });
  }

  // === 商誉减值风险 ===
  const goodwillRatio = safeDiv(financial.goodwill[n - 1], financial.equity[n - 1]) * 100;
  if (goodwillRatio > 50) {
    arguments_.push({ text: `商誉/净资产达${goodwillRatio.toFixed(1)}%，减值风险极高`, confidence: 88, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`商誉/净资产${goodwillRatio.toFixed(0)}%，减值风险极高`);
    hasCriticalRisk = true;
  } else if (goodwillRatio > 20) {
    arguments_.push({ text: `商誉/净资产${goodwillRatio.toFixed(1)}%，存在一定减值风险`, confidence: 75, type: 'oppose', evidenceType: 'fact' });
  ...
  if (latestDebtRatio > 80) {
    arguments_.push({ text: `资产负债率${latestDebtRatio.toFixed(1)}%，财务风险极高`, confidence: 85, type: 'oppose', evidenceType: 'fact' });
    keyPoints.push(`负债率${latestDebtRatio.toFixed(0)}%，财务风险极高`);
    hasCriticalRisk = true;
  } else if (latestDebtRatio > 60 && debtTrend > 5) {
    arguments_.push({ text: `资产负债率${latestDebtRatio.toFixed(1)}%且逐年上升（+${debtTrend.toFixed(1)}个百分点），杠杆持续加大`, confidence: 78, type: 'oppose', evidenceType: 'inference' });
    keyPoints.push(`负债率上升中，杠杆加大`);
  } else if (latestDebtRatio < 30) {
    arguments_.push({ text: `资产负债率${latestDebtRatio.toFixed(1)}%，财务安全垫厚实`, confidence: 78, type: 'support', evidenceType: 'fact' });
 ...
 */
export async function riskExpert(financial: FinancialData, valuation: ValuationData, info: StockInfo): Promise<ExpertOpinion> {
