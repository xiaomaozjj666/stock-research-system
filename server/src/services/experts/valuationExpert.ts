 ...
 * 规则引擎研判（LLM 不可用时降级使用）
 */
function valuationExpertRule(financial: FinancialData, valuation: ValuationData, info: StockInfo): ExpertOpinion {
  ...
  const peerAvgPE = hasPeerData
    ? safeDiv(valuation.peerComparison.reduce((sum, p) => sum + p.pe, 0), valuation.peerComparison.length)
    : 0;

  // PEG计算：用最近一年利润增速
  const latestProfitGrowth = safeDiv(financial.netProfit[n - 1] - financial.netProfit[n - 2], financial.netProfit[n - 2]) * 100;
  const peg = safeDiv(currentPE, latestProfitGrowth);

  // 股息率估算（假设分红率30%，A股平均水平，实际因公司而异）
  const assumedPayoutRatio = 0.3;
  const dividendYield = safeDiv(financial.eps[n - 1] * assumedPayoutRatio, valuation.currentPrice) * 100;

  // DCF估值：标准两阶段模型（Gordon Growth Model）
  const idx = n - 1;
  const latestFCF = financial.operatingCashFlow[idx] - Math.abs(financial.capEx?.[idx] ?? financial.totalAssets[idx] * 0.03);
  ...
  // 阶段1：高增长期（5年）
  const revenueGrowthLatest = n >= 2
    ? safeDiv(financial.revenue[idx] - financial.revenue[idx - 1], Math.abs(financial.revenue[idx - 1]))
    : 0.1;
  ...
  // 增速放缓论点（动态年份）
  const prevYearGrowth = n >= 3
    ? (safeDiv(financial.netProfit[n - 2] - financial.netProfit[n - 3], Math.abs(financial.netProfit[n - 3])) * 100)
    : 0;
  arguments_.push({
 ...
 */
export async function valuationExpert(financial: FinancialData, valuation: ValuationData, info: StockInfo): Promise<ExpertOpinion> {
