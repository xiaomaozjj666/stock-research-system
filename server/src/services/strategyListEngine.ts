...
export async function generateStrategyList(
  ...
  // 对3种策略分别运行回测
  const strategyConfigs: { name: string; type: StrategyConfig['type']; params: Record<string, number> }[] = [
    { name: '均线交叉', type: 'ma_cross', params: { shortPeriod: 5, longPeriod: 20 } },
    { name: '动量策略', type: 'momentum', params: { lookback: 20, buyThreshold: 5, sellThreshold: -3 } },
    { name: '均值回归', type: 'mean_reversion', params: { maPeriod: 20, buyDeviation: -3, sellDeviation: 3 } }
  ];
...
function getApplicableMarket(type: string): string {
  switch (type) {
    case 'ma_cross': return '趋势明确的单边行情，如牛市主升浪或熊市主跌浪';
    case 'momentum': return '强势股持续走强的高动量环境，成交量配合良好';
    case 'mean_reversion': return '震荡市或区间波动市场，价格在均值附近反复';
    default: return '未知';
  }
}
...
function getFatalWeakness(type: string): string {
  switch (type) {
    case 'ma_cross': return '震荡市中频繁产生假信号，导致连续小额亏损（whipsaw效应）';
    case 'momentum': return '动量崩溃时回撤极大，如市场风格突然切换、黑天鹅事件';
    case 'mean_reversion': return '趋势行情中逆势操作，可能遭遇单边暴跌导致巨额亏损';
    default: return '未知';
  }
}
